...
bool Tray::create(HINSTANCE hInst){
    WNDCLASSW wc{}; wc.lpfnWndProc=wnd_thunk; wc.hInstance=hInst; wc.lpszClassName=CLS;
    RegisterClassW(&wc);
    hwnd_ = CreateWindowExW(0, CLS, L"kakao_watcher", 0,0,0,0,0,
                            HWND_MESSAGE, nullptr, hInst, this);
    if(!hwnd_){ LOGE("tray window create failed %lu", GetLastError()); return false; }
...
void Tray::set_count(int armed){
    if(hwnd_) PostMessageW(hwnd_, WM_SETCNT, (WPARAM)armed, 0);
}
...
LRESULT Tray::wndproc(HWND h, UINT m, WPARAM w, LPARAM l){
    ...
    case WM_TRAY:
        if(LOWORD(l)==WM_RBUTTONUP || LOWORD(l)==WM_CONTEXTMENU){
            ...
            DestroyMenu(menu);
            if(cmd==IDM_RESCAN && onRescan) onRescan();
            else if(cmd==IDM_EXIT && onExit) onExit();
        }
        return 0;
    case WM_DESTROY:
