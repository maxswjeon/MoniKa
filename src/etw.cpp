...
static bool get_prop(EVENT_RECORD* ev, const wchar_t* name, vector<uint8_t>& out) {
    ...
    desc.ArrayIndex = 0;
    ULONG sz = 0;
    if (TdhGetPropertySize(ev, 0, nullptr, 1, &desc, &sz) != ERROR_SUCCESS || sz == 0)
        return false;
    out.resize(sz);
...
static void WINAPI record_cb(EVENT_RECORD* ev) {
    ...
    if (self)
        self->handle_record(ev);
}

void EtwProcessMonitor::handle_record(void* rec) {
    EVENT_RECORD* ev = (EVENT_RECORD*)rec;
    if (!IsEqualGUID(ev->EventHeader.ProviderId, KERNEL_PROCESS_GUID))
        return;
    if (ev->EventHeader.EventDescriptor.Id != EVID_PROCESS_START)
        return;
    ...
    if (get_prop(ev, L"ProcessID", pidBuf)) {
        if (pidBuf.size() >= 8)
            pid = (DWORD)(*(ULONGLONG*)pidBuf.data());
        else if (pidBuf.size() >= 4)
            pid = *(DWORD*)pidBuf.data();
    }
    wstring image;
...
bool EtwProcessMonitor::start(StartFn onStart) {
    ...
    if (rc == ERROR_ALREADY_EXISTS) {
        LOGE("ETW session already exists; refusing to stop another owner");
        return false;
    }
    if (rc != ERROR_SUCCESS) {
    ...
    if (th == (TRACEHANDLE)INVALID_HANDLE_VALUE) {
        LOGE("OpenTrace failed %lu", GetLastError());
        stop();
        return false;
    }
    traceHandle_ = th;
...
void EtwProcessMonitor::stop() {
    ...
    if (traceHandle_) {
        CloseTrace((TRACEHANDLE)traceHandle_);
        traceHandle_ = 0;
    }
    if (th_.joinable())
        th_.join();
...
EtwProcessMonitor::~EtwProcessMonitor() {
    stop();
}
