...
namespace sq {
    ...
    open_t        s_open=nullptr;   close_t       s_close=nullptr;  exec_t        s_exec=nullptr;
    prepare_t     s_prepare=nullptr;step_t        s_step=nullptr;   finalize_t    s_finalize=nullptr;
    bind_text_t   s_bind_text=nullptr; bind_int64_t s_bind_int64=nullptr;
    column_int_t  s_column_int=nullptr; column_text_t s_column_text=nullptr;
    bool loaded=false;

    bool load(){
        if(loaded) return true;
        HMODULE h=LoadLibraryW(L"winsqlite3.dll");
        if(!h){ LOGE("winsqlite3.dll not found (Windows 10+ required)"); return false; }
        ...
        loaded = s_open&&s_close&&s_exec&&s_prepare&&s_step&&s_finalize&&
                 s_bind_text&&s_bind_int64&&s_column_int&&s_column_text;
        if(!loaded) LOGE("winsqlite3.dll missing exports");
        return loaded;
    }
}

DekCache::~DekCache(){ if(db_) sq::s_close((sqlite3*)db_); }

bool DekCache::exec(const char* sql){
    char* err=nullptr;
    int rc=sq::s_exec((sqlite3*)db_, sql, nullptr, nullptr, &err);
    if(rc!=SQLITE_OK){ LOGE("sqlite exec failed rc=%d: %s", rc, err?err:"?"); return false; }
    return true;
}

bool DekCache::init(const wstring& dbPath){
    if(!sq::load()) return false;
    string u8 = wide_to_utf8(dbPath);
    sqlite3* db=nullptr;
    if(sq::s_open(u8.c_str(), &db)!=SQLITE_OK){ LOGE("sqlite open failed: %s", u8.c_str()); return false; }
    db_ = db;
...
bool DekCache::put_dek(const DekHit& hit){
    ...
    if(sq::s_prepare((sqlite3*)db_, sql, -1, &st, nullptr)!=SQLITE_OK) return false;
    sq::s_bind_text(st,1,hit.rel.c_str(),-1,(void(*)(void*))SQLITE_TRANSIENT);
    sq::s_bind_text(st,2,kh.c_str(),-1,(void(*)(void*))SQLITE_TRANSIENT);
    sq::s_bind_int64(st,3,hit.reserved);
    sq::s_bind_int64(st,4,now);
    sq::s_bind_int64(st,5,now);
    sq::s_step(st); sq::s_finalize(st);
    if(isNew){
        // set first_seen if null
        exec(("UPDATE dek SET first_seen=" + std::to_string(now) +
              " WHERE first_seen IS NULL;").c_str());
    }
    return isNew;
}
...
int DekCache::count_armed(){
    ...
    if(sq::s_prepare((sqlite3*)db_, "SELECT COUNT(*) FROM dek;", -1, &st, nullptr)!=SQLITE_OK)
        return 0;
    int n=0;
    if(sq::s_step(st)==SQLITE_ROW) n=sq::s_column_int(st,0);
    sq::s_finalize(st);
    return n;
}
...
void DekCache::touch_tag(const string& rel, const char* state){
    ...
    int64_t now=now_unix();
    const char* sql =
        "INSERT INTO tag(rel,last_change,change_count,state) VALUES(?,?,1,?) "
        "ON CONFLICT(rel) DO UPDATE SET last_change=excluded.last_change, "
        "change_count=tag.change_count+1, state=excluded.state;";
    sqlite3_stmt* st=nullptr;
    if(sq::s_prepare((sqlite3*)db_, sql, -1, &st, nullptr)!=SQLITE_OK) return;
    sq::s_bind_text(st,1,rel.c_str(),-1,(void(*)(void*))SQLITE_TRANSIENT);
    sq::s_bind_int64(st,2,now);
    sq::s_bind_text(st,3,state,-1,(void(*)(void*))SQLITE_TRANSIENT);
    sq::s_step(st); sq::s_finalize(st);
}
